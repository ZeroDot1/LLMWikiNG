...
- **HTTP 500 beim Speichern von Einstellungen** (`backend/api/routes/pages.py`): Beim Speichern der Einstellungen über die `settings_post`-Route fehlte die Übergabe von `ai_config` an die `render()`-Funktion, was zu einem `UndefinedError` in Jinja2 führte. `load_ai_config()` wird nun korrekt in den Render-Kontext übergeben.
...
### Fixed
- **Dashboard Wiki-Karten 404 Fix** (`templates/index.html`): Wiki-Karten auf der Startseite verlinken nun auf `ws.slug` anstelle des Anzeigenamens `ws.name`, um 404-Fehler beim Anklicken von Wikis mit Leerzeichen im Namen (z. B. *„Main Wiki“*) zu verhindern.
- **Pending Ingest Datei-Löschung & Duplikat-Schutz** (`backend/api/routes/api.py`, `backend/api/routes/pages.py`): Ingestierte Quelldateien aus `pending` werden nach erfolgreicher Verarbeitung (`returncode == 0`) nun bedingungslos gelöscht, statt an Datums-Präfix-Prüfungen zu scheitern. Verhindert das Aufstauen ausstehender Dateien und kaskadierende Datums-Präfixe.
...
- **Page Versioning / History** (`backend/services/history.py`): Dateibasierte Versionierung ohne Git. `save_version()`, `list_versions()`, `get_version()`. Maximal 30 Versionen pro Seite. Vollständig in Web-Editor, REST API (`POST /wikis/{wiki}/pages`) und MCP (`okf_write_concept`) integriert. Neue Endpoints `GET /wiki/{wiki}/{slug}/history` und `GET /wiki/{wiki}/{slug}/history/{version_id}`.
...
### Added
- **Vollständige MCP-Prompts (37 Slash-Commands)** (`backend/api/routes/mcp.py`): Jedes der 37 MCP-Tools verfügt nun über einen entsprechenden `@mcp_server.prompt` Slash-Command (`/update`, `/update-lwk`, `/updatelwk`, `/ingest`, `/backup`, `/search`, `/status`, `/lint`, `/read`, `/sync`, `/wikis`, `/pages`, `/write`, `/users`, `/api-keys`, `/backups`, etc.) für AGY, OpenCode, Cursor und Claude Code.
- **MCP SSE Configuration Fix**: Standard-Header (`headers` statt `env`) in MCP-Clients für SSE-Verbindungen (u.a. `llmwiking-okf`) korrigiert.
...
### Added
- **Persistentes Speichern & Leeren des Update-Logs (`update.log`)**:
  - Das Update-Log wird nun vor jedem Update geleert und das Gesamtergebnis dauerhaft in die Datei `update.log` im Projekt-Stammverzeichnis geschrieben.
  - Das Log kann so jederzeit auf der Festplatte eingesehen und beim Aufruf von *Settings -> Update* persistent angezeigt werden.
...
### Added
- **Verbesserungen der System Secret-Karte (`templates/settings/apikeys.html`)**:
  - Eingabe per **Enter-Taste** im Passwortfeld (`secretPassword`) löst nun direkt das Entschlüsseln (`revealSystemSecret()`) aus.
  - Dynamisches Sperren/Entsperren der Buttons *Anzeigen* und *Neu generieren* (`toggleSecretButtons()`) in Abhängigkeit davon, ob ein Passwort eingegeben wurde.

## [2.12.31] - 2026-07-27

### Fixed
- **Behebung der JS-Fehler bei Edit/Delete in *Settings -> Keys (MCP Keys)*** (`templates/settings/apikeys.html`):
  - Umstellung des `openMcpKeyEditModal`-Aufrufs auf sauberes Data-Attribut-Handling (`data-key-id`, `data-allowed-tools`) und Nutzung des Jinja2 `| safe`-Filters für `mcp_tool_groups`, um Syntax-Fehler im Inline-JavaScript zu verhindern.

## [2.12.30] - 2026-07-27

### Fixed
- **Behebung der JSONResponse-Importfehlers in `backend/main.py`**:
  - `JSONResponse` aus `starlette.responses` importiert, um den NameError beim Zurückweisen ungültiger MCP-/API-Keys in der Middleware zu beheben.

## [2.12.29] - 2026-07-27

### Fixed
- **Behebung der ASGI Double-Response-Fehler (`backend/main.py`)**:
  - Aufruf von `await response(scope, receive, send)` in `McpApiKeyMiddleware` auf sauberes Handhandling umgestellt, damit Starlette/FastAPI im Fehlerfall (z. B. ungültiger MCP/API-Key) keine doppelten ASGI-Antworten (`RuntimeError: Expected ASGI message 'http.response.body', but got 'http.response.start'`) erzeugt.
...
### Added
- **Sprachdateien-Update (`lang/de.json`, `lang/en.json`, `templates/settings/apikeys.html`)**:
  - Neue Übersetzungsschlüssel für das Bearbeiten von MCP-Keys (`mcpkeys.edit`, `mcpkeys.edit_title`, `mcpkeys.edit_desc`, `mcpkeys.save`, `mcpkeys.cancel`) hinzugefügt.
  - Template `templates/settings/apikeys.html` durchgehend auf `_('mcpkeys.*')`-Übersetzungsfunktionen umgestellt.

## [2.12.26] - 2026-07-27

### Documentation
- **Dokumentations-Update (`README.md`, `templates/docs.html`, `templates/about.html`, `templates/about_de.html`)**:
  - Aktualisierung der Feature-Übersichten, API-Tabellen und Einstellungsbeschreibungen für Audit-Log Export (JSON/CSV), Server-Backups (`.tar.xz`), Per-User MCP-Keys Bearbeitung und den interaktiven MCP Client Konfigurator.
